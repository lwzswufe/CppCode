...
bool CRedisSubscriber::Connect()
{
    ...
    // 创建事件处理线程
    int ret = pthread_create(&_event_thread, 0, &CRedisSubscriber::EventThread, this);
    if (ret != 0)
    {
...
bool CRedisSubscriber::Subscribe(const std::string &channel_name)
{
    int ret = redisAsyncCommand(_redis_context, 
        &CRedisSubscriber::CommandCallback, this, "SUBSCRIBE %s", 
        channel_name.c_str());
    if (REDIS_ERR == ret)
    {
...
void CRedisSubscriber::ConnectCallback(const redisAsyncContext *redis_context,
    int status)
{
    if (status != REDIS_OK)
    {
...
void CRedisSubscriber::DisconnectCallback(
...
// 消息接收回调函数
void CRedisSubscriber::CommandCallback(redisAsyncContext *redis_context,
    void *reply, void *privdata)
{
    ...
    {
        // printf("Recieve message:%s:%lu:%s:%lu:%s:%lu\n",
        // redis_reply->element[0]->str, redis_reply->element[0]->len,  // 类型
        // redis_reply->element[1]->str, redis_reply->element[1]->len,  // 频道
        // redis_reply->element[2]->str, redis_reply->element[2]->len); // 数据
...
void *CRedisSubscriber::EventThread(void *data)
{
    if (NULL == data)
    {
        printf(": Error!\n");
...
void *CRedisSubscriber::EventProc()
{
    sem_wait(&_event_sem);
